...
def main():
    # ip_start = input(u'请输入开始IP：(默认:127.0.0.1)')
    ip_start=''
    if ip_start == '':
        ip_start = '127.0.0.1'
        ip_end = '127.0.0.1'
    else:
        ip_end = input(u'请输入结束IP：')
        if ip_end == '':
            ip_end = '127.0.0.1'
    # s = input(u'请输入目标主机开始端口：(默认扫描常用端口)')
    s = ''
    if s == '':
        ...
        startport = int(s)
        s = input(u'请输入目标主机结束端口：(默认:65535)')
        if s == '':
            endport = 65535
        else:
            endport = int(s)
    ...
    while 1:
        ...
        for port in portList:
            print(u'正在扫描%s ：%d' % (ip_start, port))
            try:
    ...
    print(u'扫描完成，结果保存在IP_Port.txt文件中')
