...
class ExcelReport():
    ...
    def init(self, worksheet, data):
        ...
        worksheet.merge_range('A1:F1', 'All', define_format_H1)
        worksheet.merge_range('A2:F2', 'index', define_format_H2)
        worksheet.merge_range('A3:A6', 'picture', get_format_center(self.wd))

        _write_center(worksheet, "B3", 'xiamgnu', self.wd)
        _write_center(worksheet, "B4", 'ver', self.wd)
        _write_center(worksheet, "B5", 'la', self.wd)
        _write_center(worksheet, "B6", 'net', self.wd)
        ...
        _write_center(worksheet, "D3", "count", self.wd)
        _write_center(worksheet, "D4", "count", self.wd)
        _write_center(worksheet, "D5", "fail", self.wd)
        _write_center(worksheet, "D6", "date", self.wd)
        ...
        _write_center(worksheet, "F3", "f", self.wd)
    ...
    def test_detail(self, worksheet, info):
        ...
        worksheet.merge_range('A1:H1', 'd', get_format(self.wd, {'bold': True, 'font_size': 18 ,'align': 'center','valign': 'vcenter','bg_color': 'blue', 'font_color': '#ffffff'}))
        _write_center(worksheet, "A2", 'a', self.wd)
        _write_center(worksheet, "B2", 'n', self.wd)
        _write_center(worksheet, "C2", 'x', self.wd)
        _write_center(worksheet, "D2", 'URL', self.wd)
        _write_center(worksheet, "E2", 'canshu', self.wd)
        _write_center(worksheet, "F2", 'y', self.wd)
        _write_center(worksheet, "G2", 's', self.wd)
        _write_center(worksheet, "H2", 't', self.wd)
