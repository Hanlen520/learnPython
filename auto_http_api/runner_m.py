...
def get_api():
   return om.getXML("F:\\learnPython\\auto_http_api\\test4.xml", Mhttpbase.BaseHttp())
...
class TestInterfaceCase(unittest.TestCase):
    ...
    def function(self):
        temp_result = False
        hp = str_param()
        if self.index == 2: #第一个xml预留的主要是登陆
...
if __name__ == '__main__':
    ...
    data = json.loads(json.dumps(result(mresult, test_date=str(sum_time) + "m", test_sum=go.CASE_TOTAL, test_failed=go.ERROR_NUM, test_version="v2.2", test_pl="python3",
    test_net="lo", test_name=gm[0]["title"], test_success=go.SUCCESS_SUM, info=mresult.info)))
